...
var PG = {
    ...
    native: {
        upload: function(mediaFile, success, error, param,fileName) {
        ...
        audio: function(success, error, limit) {
            ...
            navigator.device.capture.captureAudio(success || $.noop, error || $.noop, {
                limit: limit || 1,
                duration: 60
            });
        },
        image: function(success, error, limit) {
            ...
            navigator.device.capture.captureImage(success || $.noop, error || $.noop, {
                limit: limit || 1
            });
        },
        camera: function(success,error,option){
            if (!navigator || !navigator.camera || !navigator.camera.getPicture) {
                $f.isFunction(error) && error();
                log('Image Error');
                return;
            }
            navigator.camera.getPicture(success || $.noop, error || $.noop,option||{});
        },
        video: function(success, error, limit) {
            ...
            navigator.device.capture.captureVideo(success || $.noop, error || $.noop, {
                limit: limit || 1
            });
        },
...
var TPL = {
    ...
    <div class="foot">\
    <div class="btngroup">\
    <button class="b-6 b-cancel"><%=cancel%></button>\
    <button class="b-6 b-confirm"><%=confirm%></button>\
    </div>\
    </div>\
    </div>\
    </div>',
...
! function($) {
    ...
    $f.pop = {
        ...
        show: function($$, cls, func) {
            ...
            $tar.show();
            setTimeout(function() {
                $mask.addClass('animated fast fadeIn').css({
                    visibility: 'visible'
                });
                $box.addClass('animated ' + cls).css({
                    visibility: 'visible'
                });
        ...
        confirm: function(tip, confirm, cancel, flag, complete) {//flag true  取消按钮触发取消，其他关闭不触发
            var _ = this,_opt = $f.isObject(tip)?tip:{tip:tip};
            ...
            return {
                ...
                hide: function(func) {
                    ...
                    _.hide(_this, 'fadeIn', 'fadeOut', function() {
                        if (!_confirm) {
                            var callback = !!flag ? complete||$.noop : cancel;
        ...
        tip: function(tip, dur, manual) {
            var _ = this;
            var $elem = $($f.TPL('<div class="pop pop-tip"><div class="box"><div class="wrap"><span class="tip">{tip}</span></div></div></div>', {
                tip: tip
            }));
            $elem.bind('touchmove', function(e) {
                e.preventDefault();
        ...
        scroll: function($tar) {
            return new IScroll($tar[0], {
                ...
                preventDefaultException: {
                    tagName: /^(A|SPAN|INPUT|TEXTAREA|BUTTON|SELECT)$/
                }
            });
        }
    };
    $f.req = {
        ...
        type: function() {
            var _ = this;
            return !!$f.db.get('type') ? $.Deferred(function(dtd) {
                dtd.resolve();
            }) : $f.ajax({
        ...
        nextUser: function() {
            var _ = this;
            return !!$f.db.get('nextTchUser') ? $.Deferred(function(dtd) {
                dtd.resolve();
            }) : $f.ajax({
                url: PG.path('nextTchUser'),
                option: {
                    userId: $f.db.get('userId'),
                    billType: (_save = $f.db.get('save')) ? _save.billType : ''
                },
                success: function(json) {
        ...
        channel: function() {
            var _ = this;
            return !!$f.db.get('channel') ? $.Deferred(function(dtd) {
                dtd.resolve();
            }) : $f.ajax({
