  ...
  $.event.special[_] = {
    setup: function() {
      $(this).off('click').on(ev.start + ' ' + ev.end, function(e) {
         ...
         */
         log(ev.X,ev.E.pageX,ev.Y,ev.E.pageY);
        if (
          ev.target === e.target &&
          ((new Date().getTime() - ev.time) < 500) &&
  ...
  $.event.special[_] = {
    setup: function() {
      $(this).off('click').on(ev.start + ' ' + ev.end, function(e) {
        ...
        ev.Y = ev.E.pageY;
        var _this = this,_e = e;
        ev.timer = setTimeout(function(){
        	ev.timer = null;
        	_e.type = _;
        	_e.pageX = ev.E.pageX;
          	_e.pageY = ev.E.pageY;
        	$.event.dispatch.call(_this, _e);
        },550);
      }).on(ev.end, function(e) {
        /**
         * Compare property values of initial phase with properties
         ...
         */
        if (
          ev.target === e.target && ((new Date().getTime() - ev.time) < 550 && !!ev.timer )
        ) {
