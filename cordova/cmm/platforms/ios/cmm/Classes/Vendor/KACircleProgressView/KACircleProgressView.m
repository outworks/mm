...
@property (nonatomic, strong) CAShapeLayer * progressBar2;
@end
...
- (void)createLayersWithType:(KACircleProgressViewType)type{
    ...
    self.progressBar2 = [CAShapeLayer layer];
    self.progressBar2.lineWidth = lineWidthOfProgressBar;
    self.progressBar2.lineCap = kCALineCapSquare;
    self.progressBar2.strokeColor = [UIColor cyanColor].CGColor;
    self.progressBar2.fillColor = [UIColor clearColor].CGColor;
    [self.layer addSublayer:self.progressBar2];
...
- (void)setColorOfProgressBar:(UIColor *)color{
    ...
    [self.progressBar2 setStrokeColor:color.CGColor];
    [self setNeedsDisplay];
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat end = (self.progress * (float)M_PI) + start;
    ...
    start = (- M_PI_2) + ((1-self.progress) * (float)M_PI) ;
    end = (M_PI_2);
    UIBezierPath *processBackgroundPath2 = [UIBezierPath bezierPath];
    processBackgroundPath2.lineWidth = lineWidthOfProgressBar;
    [processBackgroundPath2 addArcWithCenter: CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2) radius:radius startAngle:start endAngle:end clockwise:YES];

    self.progressBar2.path = processBackgroundPath2.CGPath;
}
